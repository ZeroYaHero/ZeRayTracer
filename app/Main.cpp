...
// 6. The above "multiple rays" are distributed in different strategies. "Regular" assumes constant offsets for each pixel, random is randomized (with different stratgies to pick those random points!).
...
// 12. Lambertian distribution is where you generate the bounce direction in a unit sphere *above* the hit point by a unit direction of the normal (and that way most of the the points will be directly above the normal)
// 13. Adding a randomized vector to the bounce/reflection will make a reflection "fuzzy"
// 14. Refraction is where the ray goes "inside" so for things like glass. These are dialectric materials and they require the knowledge of refractive indexes and mediums.
// 15. Total internal refraction is where the ray will always refract, but this is not always the case for real world materials. In example, spheres are perfectly round/symmetric so the rays always exit at the same angle.